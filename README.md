...
Подготовка среды: Убедитесь, что установлен Microsoft SQL Server и SQL Server Management Studio (SSMS)

Создание базы: В SSMS выполните:

sql
CREATE DATABASE WarehouseManagement;
Загрузка структуры: Выполните скрипт database/Общая схема.sql

Наполнение данными: Используйте скрипт database/Тестирование.sql для тестовых данных

Проверка работы: Используйте демонстрационные запросы:

sql
-- Пример из Демонстрация запросов.sql
...
-- Проверка процедур из Просмотр процедур.sql
EXEC ПримерПроцедуры;

📊 Доступные скрипты
📝 Основные скрипты
Общая схема.sql - Основная структура базы данных

Описание таблиц.md - Документация по таблицам и связям

🔍 Аналитические скрипты
Демонстрация запросов.sql - Примеры рабочих запросов

Запросы для анализа произвола.sql - Выявление аномалий в данных

⚙️ Обслуживание
Обслуживание базы данных.sql - Оптимизация и обслуживание

Просмотр процедур.sql - Управление хранимыми процедурами

Скрипты резервного копирования.sql - Бэкапы и восстановление

🧪 Тестирование
Тестирование.sql - Проверка функциональности и производительности
...
Описание БД: database/README.md

Структура таблиц: database/Описание таблиц.md
